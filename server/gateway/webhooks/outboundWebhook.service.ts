...
export async function enqueuePaymentIntentEvent(
  paymentIntentId: string,
  eventType: OutboundPaymentEventType
): Promise<void> {
  ...
  const payload: OutboundWebhookPayload = {
    ...
    data: {
      paymentIntent: {
        ...
        orderReference: paymentIntent.orderReference ?? undefined,
        providerReference: paymentIntent.providerReference ?? undefined,
      },
    },
  };

  await db.insert(outboundWebhookEvents).values({
    eventId,
    merchantId: paymentIntent.merchantId,
    paymentIntentId,
    eventType,
...
async function deliverOne(eventId: number): Promise<void> {
  ...
  const rows = await db
    .select({ event: outboundWebhookEvents, endpoint: merchantWebhookEndpoints })
    .from(outboundWebhookEvents)
    .innerJoin(
      merchantWebhookEndpoints,
      eq(outboundWebhookEvents.merchantId, merchantWebhookEndpoints.merchantId)
    )
    .where(and(eq(outboundWebhookEvents.id, eventId), eq(merchantWebhookEndpoints.enabled, 1)))
    .limit(1);
  ...
  if (!row) {
    await db.update(outboundWebhookEvents).set({
      status: "dead_letter",
      lastError: "merchant_webhook_endpoint_unavailable",
      updatedAt: new Date(),
  ...
  const secret = decryptJson<string>(
    row.endpoint.secret as EncryptedEnvelope,
    `merchant-webhook:${row.endpoint.merchantId}`
  );
  ...
  try {
    ...
    await db.update(outboundWebhookEvents).set({
      status: retryable && !exhausted ? "retrying" : "dead_letter",
      nextAttemptAt: new Date(Date.now() + calculateBackoffSeconds(attempt) * 1000),
      lastResponseStatus: response.status,
      lastResponseBody: responseBody,
    ...
    await db.update(outboundWebhookEvents).set({
      ...
      nextAttemptAt: new Date(Date.now() + calculateBackoffSeconds(attempt) * 1000),
      lastError: error instanceof Error ? error.message.slice(0, 512) : "delivery_failed",
      updatedAt: new Date(),
    }).where(eq(outboundWebhookEvents.id, eventId));
  }
}
...
export async function processOutboundWebhookQueue(limit = 25): Promise<void> {
  ...
  const due = await db.select({ id: outboundWebhookEvents.id }).from(outboundWebhookEvents).where(
    and(
      inArray(outboundWebhookEvents.status, ["queued", "retrying"]),
...
export function startOutboundWebhookProcessor(intervalMs = 5000): ReturnType<typeof setInterval> {
  ...
  const timer = setInterval(() => {
    processOutboundWebhookQueue().catch((error) => {
      console.error("[OutboundWebhook] Queue processing failed:", error);
    });
  }, intervalMs);
  timer.unref?.();
  return timer;
